...
class DeletionTestCase(NamedTestCase):
    ...
    def test_combined_del_123(self):
        db.session.delete(
            db.session.query(Named).filter_by(name=u"root1").one())
        db.session.delete(
            db.session.query(Named).filter_by(name=u"child212").one())
        db.session.delete(
            db.session.query(Named).filter_by(name=u"child23").one())
        db.session.commit()
