...
class NamedTestCase(TreeTestMixin, unittest.TestCase):
    ...
    def test_tree_id_field(self):
        ...
        self.assertIsInstance(
            Named.tree.tree_id_field.type, sqlalchemy_tree.types.TreeIdType)
    ...
    def test_left_field(self):
        ...
        self.assertIsInstance(
            Named.tree.left_field.type, sqlalchemy_tree.types.TreeLeftType)
    ...
    def test_right_field(self):
        ...
        self.assertIsInstance(
            Named.tree.right_field.type, sqlalchemy_tree.types.TreeRightType)
    ...
    def test_depth_field(self):
        ...
        self.assertIsInstance(
            Named.tree.depth_field.type, sqlalchemy_tree.types.TreeDepthType)
    ...
    def test_query_root_nodes(self):
        ...
        self.assertEqual(
            expected,
            [x.name
             for x in Named.tree.query_root_nodes(session=db.session).order_by(
                 Named.name).all()])
    ...
    def test_filter_root_node_by_tree_id(self):
        ...
        def _process_node(root_name, node_name, children):
            ...
            self.assertEqual(
                root_name, db.session.query(Named).filter(Named.tree.
                                                          filter_root_node_by_tree_id(node.tree_id)).one().name)
    ...
    def test_query_root_node_by_tree_id(self):
        ...
        def _process_node(root_name, node_name, children):
            node = db.session.query(Named).filter_by(name=node_name).one()
            self.assertEqual(
                root_name, Named.tree.query_root_node_by_tree_id(node.tree_id,
                                                                 session=db.session).one().name)
    ...
    def test_query_root_node_of_node(self):
        ...
        def _process_node(root_name, node_name, children):
            node = db.session.query(Named).filter_by(name=node_name).one()
            self.assertEqual(
                root_name, Named.tree.query_root_node_of_node(node, session=db.
                                                              session).one().name)
            self.assertEqual(root_name,
    ...
    def test_filter_ancestors(self):
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(
                result['ancestors'] + [obj.name], map(lambda x: x.name, db.
                                                      session.query(Named).filter(obj.tree.filter_ancestors(
                                                          include_self=True)).order_by(Named.tree).all()))
    ...
    def test_query_ancestors(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['ancestors'], map(lambda x: x.name, obj.tree.
                                         query_ancestors().order_by(Named.tree).all()))
            self.assertEqual(
                result['ancestors'] + [obj.name], map(lambda x: x.name, obj.
                                                      tree.query_ancestors(include_self=True).order_by(Named.tree).
                                                      all()))
    ...
    def test_filter_parent(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['parent'], map(lambda x: x.name,
    ...
    def test_filter_parent_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['parent'], map(lambda x: x and x.name or None, db.
                                      session.query(Named).filter(Named.tree.filter_parent_of_node(
                                          obj)).all()))
    ...
    def test_filter_siblings(self):
        ...
        for pattern in self.result_static:
            name, result = pattern
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(result['previous-siblings'] + result
                             ['next-siblings'], map(lambda x: x.name, db.
                                                    session.query(Named).filter(obj.tree.
                                                                                filter_siblings()).all()))
            self.assertEqual(
                result['previous-siblings'] + [name] + result['next-siblings'],
                map(lambda x: x.name, db.session.query(Named).filter(obj.tree.
                                                                     filter_siblings(include_self=True)).all()))
    ...
    def test_query_siblings(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(result['previous-siblings'] + result
                             ['next-siblings'], map(lambda x: x.name, obj.tree.
                                                    query_siblings().all()))
            self.assertEqual(
                result['previous-siblings'] + [name] + result['next-siblings'],
                map(lambda x: x.name, obj.tree.query_siblings(
    ...
    def test_filter_previous_siblings(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['previous-siblings'], map(lambda x: x.name, db.session.
                                                 query(Named).filter(obj.tree.filter_previous_siblings()).all()))
            self.assertEqual(
                result['previous-siblings'] + [name], map(lambda x: x.name, db.
                                                          session.query(Named).filter(obj.tree.filter_previous_siblings(
                                                              include_self=True)).all()))
    ...
    def test_query_previous_siblings(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['previous-siblings'], map(lambda x: x.name, obj.tree.
                                                 query_previous_siblings().all()))
            self.assertEqual(
                result['previous-siblings'] + [name], map(lambda x: x.name, obj.
                                                          tree.query_previous_siblings(include_self=True).all()))
    ...
    def test_filter_next_siblings(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['next-siblings'], map(lambda x: x.name, db.session.
                                             query(Named).filter(obj.tree.filter_next_siblings()).all()))
            self.assertEqual(
                [name] + result['next-siblings'], map(lambda x: x.name, db.
                                                      session.query(Named).filter(obj.tree.filter_next_siblings(
                                                          include_self=True)).all()))
    ...
    def test_query_next_siblings(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['next-siblings'], map(lambda x: x.name, obj.tree.
                                             query_next_siblings().all()))
            self.assertEqual(
                [name] + result['next-siblings'], map(lambda x: x.name, obj.
                                                      tree.query_next_siblings(include_self=True).all()))
    ...
    def test_filter_siblings_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(result['previous-siblings'] + result
                             ['next-siblings'], map(lambda x: x.name, db.
                                                    session.query(Named).filter(Named.tree.
                                                                                filter_siblings_of_node(obj)).order_by(Named.
                                                                                                                       tree).all()))
            self.assertEqual(result['previous-siblings'] + [obj.name] + result
                             ['next-siblings'], map(lambda x: x.name, db.
                                                    session.query(Named).filter(Named.tree.
                                                                                filter_siblings_of_node(obj,
                                                                                                        include_self=True)).order_by(Named.tree).all()))
        ...
        for results in permutations(self.result_static, 2):
            ...
            nodes = [
                db.session.query(Named).filter_by(name=x).one()
                for x in names]
            siblings = [
                set(x[1]['previous-siblings'] + x[1]['next-siblings'])
                for x in results]
            siblings2 = map(
                lambda x: x[1].union(set([x[0]])), zip(names, siblings))
            ...
            self.assertEqual(
                union, set(map(lambda node: node.name, db.session.query(Named).
                               filter(Named.tree.filter_siblings_of_node(*nodes)).all())))
            self.assertEqual(
                union2, set(map(lambda node: node.name,
                                db.session.query(Named).filter(Named.tree.
                                                               filter_siblings_of_node(*nodes, include_self=True)).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, db.session.query(
                    Named).filter(Named.tree.filter_siblings_of_node(*nodes,
                                                                     disjoint=False)).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, db.session.
                                       query(Named).filter(Named.tree.filter_siblings_of_node(*
                                                                                              nodes, include_self=True, disjoint=False)).all())))
    ...
    def test_query_siblings_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(result['previous-siblings'] + result
                             ['next-siblings'], map(lambda x: x.name, Named.
                                                    tree.query_siblings_of_node(obj).order_by(Named.
                                                                                              tree).all()))
            self.assertEqual(
                result['previous-siblings'] + [obj.name] + result
                ['next-siblings'], map(lambda x: x.name, Named.tree.
                                       query_siblings_of_node(obj, include_self=True).order_by(Named.
                                                                                               tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            self.assertEqual(union,
                             set(map(
                                 ...
                                 .all())))
            self.assertEqual(
                union2, set(map(lambda node: node.name, Named.tree.
                                query_siblings_of_node(*nodes, include_self=True).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, Named.tree.
                                      query_siblings_of_node(*nodes, disjoint=False).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, Named.tree.
                                       query_siblings_of_node(*nodes, include_self=True,
                                                              disjoint=False).all())))
    ...
    def test_filter_previous_siblings_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(
                result['previous-siblings'], map(lambda x: x.name, db.session.
                                                 query(Named).filter(Named.tree.
                                                                     filter_previous_siblings_of_node(obj)).order_by(Named.tree).
                                                 all()))
            self.assertEqual(
                result['previous-siblings'] + [obj.name], map(lambda x: x.name,
                                                              db.session.query(Named).filter(Named.tree.
                                                                                             filter_previous_siblings_of_node(obj, include_self=True)).
                                                              order_by(Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            self.assertEqual(
                union2, set(map(lambda node: node.name,
                                db.session.query(Named).filter(Named.tree.
                                                               filter_previous_siblings_of_node(*nodes,
                                                                                                include_self=True)).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, db.session.query(
                    Named).filter(Named.tree.filter_previous_siblings_of_node(*
                                                                              nodes, disjoint=False)).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, db.session.
                                       query(Named).filter(Named.tree.
                                                           filter_previous_siblings_of_node(*nodes, include_self=True,
                                                                                            disjoint=False)).all())))
    ...
    def test_query_previous_siblings_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['previous-siblings'], map(lambda x: x.name, Named.tree.
                                                 query_previous_siblings_of_node(obj).order_by(Named.tree).all()))
            self.assertEqual(
                result['previous-siblings'] + [obj.name], map(lambda x: x.name,
        ...
        for results in permutations(self.result_static, 2):
            ...
            intersection2 = reduce(lambda l, r: l.intersection(r), siblings2)
            self.assertEqual(
                union, set(map(lambda node: node.name, Named.tree.
                               query_previous_siblings_of_node(*nodes).all())))
            self.assertEqual(
                union2, set(map(lambda node: node.name, Named.tree.
                                query_previous_siblings_of_node(*nodes, include_self=True).
                                all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, Named.tree.
                                      query_previous_siblings_of_node(*nodes,
                                                                      disjoint=False).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, Named.tree.
                                       query_previous_siblings_of_node(*nodes, include_self=True,
    ...
    def test_filter_next_siblings_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(
                result['next-siblings'], map(lambda x: x.name, db.session.
                                             query(Named).filter(Named.tree.filter_next_siblings_of_node(
                                                 obj)).order_by(Named.tree).all()))
            self.assertEqual(
                [obj.name] + result['next-siblings'], map(lambda x: x.name, db.
                                                          session.query(Named).filter(Named.tree.
                                                                                      filter_next_siblings_of_node(obj, include_self=True)).
                                                          order_by(Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            self.assertEqual(
                union, set(map(lambda node: node.name, db.session.query(Named).
                               filter(Named.tree.filter_next_siblings_of_node(*nodes)).all())))
            self.assertEqual(
                union2, set(map(lambda node: node.name,
                                db.session.query(Named).filter(Named.tree.
                                                               filter_next_siblings_of_node(*nodes, include_self=True)).
                                all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, db.session.query(
                    Named).filter(Named.tree.filter_next_siblings_of_node(*
                                                                          nodes, disjoint=False)).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, db.session.
                                       query(Named).filter(Named.tree.filter_next_siblings_of_node(
    ...
    def test_query_next_siblings_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(result['next-siblings'],
                             map(lambda x: x.name,
                                 ...
                                 .order_by(Named.tree).all()))
            self.assertEqual(
                [obj.name] + result['next-siblings'], map(lambda x: x.name,
                                                          Named.tree.query_next_siblings_of_node(obj,
                                                                                                 include_self=True).order_by(Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            self.assertEqual(
                union2, set(map(lambda node: node.name, Named.tree.
                                query_next_siblings_of_node(*nodes,
                                                            include_self=True).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, Named.tree.
                                      query_next_siblings_of_node(*nodes, disjoint=False).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, Named.tree.
                                       query_next_siblings_of_node(*nodes, include_self=True,
                                                                   disjoint=False).all())))
    ...
    def test_filter_ancestors_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['ancestors'], map(lambda x: x.name, db.session.query(
                    Named).filter(Named.tree.filter_ancestors_of_node(obj)).
                    order_by(Named.tree).all()))
            self.assertEqual(
                result['ancestors'] + [obj.name], map(lambda x: x.name, db.
                                                      session.query(Named).filter(Named.tree.
                                                                                  filter_ancestors_of_node(obj, include_self=True)).order_by(
                                                          Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            ancestors2 = map(
                lambda x: x[1].union(set([x[0]])), zip(names, ancestors))
            ...
            self.assertEqual(
                union, set(map(lambda node: node.name, db.session.query(Named).
                               filter(Named.tree.filter_ancestors_of_node(*nodes)).all())))
            self.assertEqual(
                union2, set(map(lambda node: node.name,
                                db.session.query(Named).filter(Named.tree.
                                                               filter_ancestors_of_node(*nodes,
                                                                                        include_self=True)).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, db.session.query(
                    Named).filter(Named.tree.filter_ancestors_of_node(*nodes,
                                                                      disjoint=False)).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, db.session.
                                       query(Named).filter(Named.tree.filter_ancestors_of_node(*
                                                                                               nodes, include_self=True, disjoint=False)).all())))
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(result['ancestors'],
                             map(lambda x: x.name,
                                 ...
                                 .order_by(Named.tree).all()))
            self.assertEqual(
                result['ancestors'] + [obj.name], map(lambda x: x.name, Named.
                                                      tree.query_ancestors_of_node(obj, include_self=True).order_by(
                                                          Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            self.assertEqual(
                union2, set(map(lambda node: node.name, Named.tree.
                                query_ancestors_of_node(*nodes, include_self=True).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, Named.tree.
                                      query_ancestors_of_node(*nodes, disjoint=False).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, Named.tree.
                                       query_ancestors_of_node(*nodes, include_self=True,
                                                               disjoint=False).all())))
    ...
    def test_query_children(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['children'], map(lambda x: x.name, obj.tree.
                                        query_children().order_by(Named.tree).all()))
    ...
    def test_filter_children_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['children'], map(lambda x: x.name, db.session.query(
                    Named).filter(Named.tree.filter_children_of_node(obj)).
                    order_by(Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            self.assertEqual(
                union, set(map(lambda node: node.name, db.session.query(Named).
                               filter(Named.tree.filter_children_of_node(*nodes)).all())))
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(result['descendants'],
                             map(lambda x: x.name,
                                 ...
                                 .order_by(Named.tree).all()))
            self.assertEqual(
                [obj.name] + result['descendants'], map(lambda x: x.name, db.
                                                        session.query(Named).filter(obj.tree.filter_descendants(
                                                            include_self=True)).order_by(Named.tree).all()))
    ...
    def test_query_descendants(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['descendants'], map(lambda x: x.name, obj.tree.
                                           query_descendants().order_by(Named.tree).all()))
            self.assertEqual(
                [obj.name] + result['descendants'], map(lambda x: x.name, obj.
                                                        tree.query_descendants(include_self=True).order_by(Named.tree).
                                                        all()))
    ...
    def test_filter_descendants_of_node(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['descendants'], map(lambda x: x.name, db.session.query(
                    Named).filter(Named.tree.filter_descendants_of_node(obj)).
                    order_by(Named.tree).all()))
            self.assertEqual(
                [obj.name] + result['descendants'], map(lambda x: x.name, db.
                                                        session.query(Named).filter(Named.tree.
                                                                                    filter_descendants_of_node(obj, include_self=True)).order_by(
                                                            Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            descendants2 = map(
                lambda x: x[1].union(set([x[0]])), zip(names, descendants))
            ...
            intersection2 = reduce(
                lambda l, r: l.intersection(r), descendants2)
            self.assertEqual(
                union, set(map(lambda node: node.name, db.session.query(Named).
                               filter(Named.tree.filter_descendants_of_node(*nodes)).all())))
            self.assertEqual(
                union2, set(map(lambda node: node.name,
                                db.session.query(Named).filter(Named.tree.
                                                               filter_descendants_of_node(*nodes,
                                                                                          include_self=True)).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, db.session.query(
                    Named).filter(Named.tree.filter_descendants_of_node(*nodes,
                                                                        disjoint=False)).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, db.session.
                                       query(Named).filter(Named.tree.filter_descendants_of_node(*
                                                                                                 nodes, include_self=True, disjoint=False)).all())))
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(result['descendants'],
                             map(lambda x: x.name,
                                 ...
                                 .order_by(Named.tree).all()))
            self.assertEqual(
                [obj.name] + result['descendants'], map(lambda x: x.name, Named.
                                                        tree.query_descendants_of_node(obj, include_self=True).
                                                        order_by(Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            self.assertEqual(
                union2, set(map(lambda node: node.name, Named.tree.
                                query_descendants_of_node(*nodes, include_self=True).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, Named.tree.
                                      query_descendants_of_node(*nodes, disjoint=False).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, Named.tree.
                                       query_descendants_of_node(*nodes, include_self=True,
                                                                 disjoint=False).all())))
    ...
    def get_descendant_count(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                len(result['descendants']), obj.tree.get_descendant_count())
    ...
    def test_filter_leaf_nodes(self):
        ...
        for pattern in self.result_static:
            ...
            if result['descendants']:
                ...
                expected = [obj.name]
            self.assertEqual(
                expected + result['leaf-nodes'], map(lambda x: x.name, db.
                                                     session.query(Named).filter(obj.tree.filter_leaf_nodes(
                                                         include_self=True)).order_by(Named.tree).all()))
        expected = set(map(
            lambda x: x[0],
    ...
    def test_query_leaf_nodes(self):
        ...
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                result['leaf-nodes'], map(lambda x: x.name, obj.tree.
                                          query_leaf_nodes().order_by(Named.tree).all()))
            if result['descendants']:
                expected = []
            else:
                expected = [obj.name]
            self.assertEqual(
                expected + result['leaf-nodes'], map(lambda x: x.name, obj.tree.
                                                     query_leaf_nodes(include_self=True).order_by(Named.tree).all()))
        expected = set(map(
            lambda x: x[0],
            filter(lambda x: not x[1]['descendants'], self.result_static)))
        self.assertEqual(
            expected, set(map(lambda node: node.name, Named.tree.
                              query_leaf_nodes(session=db.session).all())))
    ...
    def test_query_leaf_nodes_by_tree_id(self):
        ...
        for pattern in self.name_pattern:
            ...
            self.assertEqual(
                expected, set(map(lambda node: node.name, Named.tree.
                                  query_leaf_nodes_by_tree_id(params['id'],
                                                              session=db.session).all())))
    ...
    def test_filter_leaf_nodes_of_node(self):
        ...
        for pattern in self.result_static:
            name, result = pattern
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(result['leaf-nodes'], map(lambda x: x.name, db.
                                                       session.query(Named).filter(Named.tree.
                                                                                   filter_leaf_nodes_of_node(obj)).order_by(Named.
                                                                                                                            tree).all()))
            self.assertEqual(
                result['leaf-nodes'] or[obj.name], map(lambda x: x.name, db.
                                                       session.query(Named).filter(Named.tree.
                                                                                   filter_leaf_nodes_of_node(obj, include_self=True)).order_by(
                                                           Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            leaf_nodes2 = map(
                lambda x: x[1] or set([x[0]]), zip(names, leaf_nodes))
            ...
            self.assertEqual(
                union, set(map(lambda node: node.name, db.session.query(Named).
                               filter(Named.tree.filter_leaf_nodes_of_node(*nodes)).all())))
            self.assertEqual(
                union2, set(map(lambda node: node.name,
                                db.session.query(Named).filter(Named.tree.
                                                               filter_leaf_nodes_of_node(*nodes,
                                                                                         include_self=True)).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, db.session.query(
                    Named).filter(Named.tree.filter_leaf_nodes_of_node(*nodes,
                                                                       disjoint=False)).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, db.session.
                                       query(Named).filter(Named.tree.filter_leaf_nodes_of_node(*
                                                                                                nodes, include_self=True, disjoint=False)).all())))
        ...
        for pattern in self.result_static:
            ...
            self.assertEqual(result['leaf-nodes'],
                             map(lambda x: x.name,
                                 ...
                                 .order_by(Named.tree).all()))
            self.assertEqual(
                result['leaf-nodes'] or[obj.name], map(lambda x: x.name, Named.
                                                       tree.query_leaf_nodes_of_node(obj, include_self=True).
                                                       order_by(Named.tree).all()))
        # permutations() is used instead of combinations() to ensure that the
        # result is irrespective of the ordering of the nodes:
        for results in permutations(self.result_static, 2):
            ...
            self.assertEqual(
                union2, set(map(lambda node: node.name, Named.tree.
                                query_leaf_nodes_of_node(*nodes, include_self=True).all())))
            self.assertEqual(
                intersection, set(map(lambda node: node.name, Named.tree.
                                      query_leaf_nodes_of_node(*nodes, disjoint=False).all())))
            self.assertEqual(
                intersection2, set(map(lambda node: node.name, Named.tree.
                                       query_leaf_nodes_of_node(*nodes, include_self=True,
                                                                disjoint=False).all())))
    ...
    def test_is_leaf_node(self):
        for pattern in self.result_static:
            ...
            obj = db.session.query(Named).filter_by(name=name).one()
            self.assertEqual(
                not bool(result['children']), obj.tree.is_leaf_node)
    ...
    def test_siblings_of(self):
        for results in permutations(self.result_static, 3):
            ...
            nodes = [
                db.session.query(Named).filter_by(name=x).one()
                for x in names]
            siblings = results[0][1]['previous-siblings'] + \
                results[0][1]['next-siblings']
            siblings2 = results[0][1]['previous-siblings'] + \
                names[:1] + results[0][1]['next-siblings']
            self.assertEqual(
